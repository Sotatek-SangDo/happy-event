...
public function cmsmasters_generate_front_css($css) {
...
public function cmsmasters_icon_list_item($atts, $content = null) {
	...
	if ($this->icon_list_items_atts['list_type'] == 'block') {
		...
		(($classes != '') ? ' ' . esc_attr($classes) : '') . 
		'"><' . esc_attr($this->icon_list_items_atts['list_heading']) . '>' . esc_html($title) . '</' . esc_attr($this->icon_list_items_atts['list_heading']) . '></li>';
	}
}
...
public function cmsmasters_google_map_markers($atts, $content = null) {
    ...
    $maps_out .= '<div><script type="text/javascript">' . 
        'jQuery(document).ready(function () { ' . 
            'jQuery("#google_map_' . esc_attr($unique_id) . '").gMap( { ';
				if ($address_type == 'address') {
					$maps_out .= 'address: "' . esc_attr($address) . '", ' ;
				} else {
					$maps_out .= 'markers: {' . 
						(($latitude != '') ? 'latitude: ' . esc_attr($latitude) . ', ' : '') . 
						(($longitude != '') ? 'latitude: ' . esc_attr($longitude) . ' ' : '') . 
					'},';
				}
				$maps_out .= 'maptype: "' . esc_attr($type) . '", ' . 
				'zoom: ' . esc_attr($zoom) . ', ' . 
				(($scroll_wheel == 'true') ? 'scrollwheel: ' . esc_attr($scroll_wheel) . ', ' : '') . 
				(($double_click_zoom == 'true') ? 'doubleClickZoom: ' . esc_attr($double_click_zoom) . ', ' : '') . 
				'controls: {' . 
					(($pan_control == 'true') ? 'panControl: ' . esc_attr($pan_control) . ', ' : '') . 
					(($zoom_control == 'true') ? 'zoomControl: ' . esc_attr($zoom_control) . ', ' : '') . 
					(($map_type_control == 'true') ? 'mapTypeControl: ' . esc_attr($map_type_control) . ', ' : '') . 
					(($scale_control == 'true') ? 'scaleControl: ' . esc_attr($scale_control) . ', ' : '') . 
					(($street_view_control == 'true') ? 'streetViewControl: ' . esc_attr($street_view_control) . ', ' : '') . 
					(($overview_map_control == 'true') ? 'overviewMapControl: ' . esc_attr($overview_map_control) . ', ' : '') . 
				'},';
				$maps_out .= 'markers: [' . 
				...
				if ($tweets != '') {
					...
					echo '<div class="cmsmasters_notice cmsmasters_notice_error cmsmasters_theme_icon_cancel">' . "\n" . 
						'<div class="notice_content">' . "\n" . 
							'<p>' . esc_html__('Please add your Twitter API keys', 'cmsmasters_content_composer') . ', ' . '<a target="_blank" href="http://docs.cmsmasters.net/admin2/twitter-functionality/">' . esc_html__('read more how', 'cmsmasters_content_composer') . '</a></p>' . "\n" . 
						'</div>' . "\n" . 
					'</div>' . "\n";
				}
...
jQuery(document).ready(function () {
	(function ($) {
		if ($('#blog_" . esc_attr($unique_id) . "').find('article').length == '0') {
			return false;
		}
...
public function cmsmasters_portfolio($atts, $content = null) {
	...
	$more_text = ($more_text != '') ? $more_text : esc_html__('Load More Projects', 'cmsmasters');
	
	$filter_text = ($filter_text != '') ? $filter_text : esc_html__('Filter', 'cmsmasters');
	
	$filter_cats_text = ($filter_cats_text != '') ? $filter_cats_text : esc_html__('All Categories', 'cmsmasters');
	
	$sorting_name_text = ($sorting_name_text != '') ? $sorting_name_text : esc_html__('Name', 'cmsmasters');
	
	$sorting_date_text = ($sorting_date_text != '') ? $sorting_date_text : esc_html__('Date', 'cmsmasters');
...
jQuery(document).ready(function () {
	(function ($) {
		if ($('#portfolio_" . esc_attr($unique_id) . "').find('article').length == '0') {
			return false;
		}
