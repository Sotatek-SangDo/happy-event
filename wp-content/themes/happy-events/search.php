...
if (!have_posts()) : 
	echo '<div class="cmsmasters_search_zero">' . 
		'<h4>' . esc_html__('Sorry, but nothing matched your search terms. Please try again with some different keywords.', 'happy-events') . '</h4>' . 
		'<br />';
			...
			<div class="cmsmasters_archive_item_inner">
				...
				if (!post_password_required() && has_post_thumbnail()) {
					echo '<div class="cmsmasters_archive_item_img_wrap">';
						happy_events_thumb(get_the_ID(), 'cmsmasters-square-thumb');
					echo '</div>';
				}
				?>
				<div class="cmsmasters_archive_item_cont_wrap">
					...
					if (get_post_type() == 'project' || $current_tax != '') {
						...
						if($cmsmasters_project_duration != '') {
							echo '<div class="cmsmasters_archive_item_service_duration">'.
						...
						if($cmsmasters_project_participants != '') {
							echo '<div class="cmsmasters_archive_item_service_participants">'.
						...
						echo '<div class="cmsmasters_archive_item_info entry-meta">';
							...
							if (get_post_type() == 'post') {
								echo '<span class="cmsmasters_archive_item_user_name">' . 
									...
									'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" rel="author" title="' . esc_attr__('Posts by', 'happy-events') . ' ' . get_the_author_meta('display_name') . '">' . get_the_author_meta('display_name') . '</a>' . 
								'</span>';
							}
							...
							if ($current_tax != '') {
								echo '<span class="cmsmasters_archive_item_category">' . 
									...
									happy_events_get_the_category_list(get_the_ID(), $current_tax, ', ') . 
								'</span>';
							}
					...
					if (get_post_type() == 'project' || $current_tax != '') {
						...
						if($cmsmasters_project_price != '') {
							echo '<div class="cmsmasters_archive_item_service_price">';						
								happy_events_project_price($cmsmasters_project_price, 'page');
							'</div>';
						}
					}
					?>
