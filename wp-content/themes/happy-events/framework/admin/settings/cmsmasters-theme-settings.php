...
add_action('admin_menu', 'happy_events_add_menu');
add_action('admin_init', 'happy_events_register_settings');
...
function happy_events_get_settings() {
	$output = array();
	
	
	$page = happy_events_get_admin_page();
	...
	case 'cmsmasters-settings':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'happy-events';
		$cmsmasters_settings_page_title = esc_html__('Theme General Settings', 'happy-events');
		$cmsmasters_page_sections = apply_filters('cmsmasters_options_general_sections_filter', happy_events_options_general_sections(), $tab);
		$cmsmasters_page_fields = apply_filters('cmsmasters_options_general_fields_filter', happy_events_options_general_fields(), $tab);
		$cmsmasters_page_tabs = apply_filters('cmsmasters_options_general_tabs_filter', happy_events_options_general_tabs());
	...
	case 'cmsmasters-settings' . '-font':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'happy-events' . '_font';
		$cmsmasters_settings_page_title = esc_html__('Theme Fonts Settings', 'happy-events');
		$cmsmasters_page_sections = apply_filters('cmsmasters_options_font_sections_filter', happy_events_options_font_sections(), $tab);
		$cmsmasters_page_fields = apply_filters('cmsmasters_options_font_fields_filter', happy_events_options_font_fields(), $tab);
		$cmsmasters_page_tabs = apply_filters('cmsmasters_options_font_tabs_filter', happy_events_options_font_tabs());
	...
	case 'cmsmasters-settings' . '-color':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'happy-events' . '_color';
		$cmsmasters_settings_page_title = esc_html__('Theme Color Schemes', 'happy-events');
		$cmsmasters_page_sections = happy_events_options_color_sections();
		$cmsmasters_page_fields = apply_filters('cmsmasters_options_color_fields_filter', happy_events_options_color_fields(), $tab);
		$cmsmasters_page_tabs = happy_events_options_color_tabs();
	...
	case 'cmsmasters-settings' . '-element':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'happy-events' . '_element';
		$cmsmasters_settings_page_title = esc_html__('Theme Elements Settings', 'happy-events');
		$cmsmasters_page_sections = apply_filters('cmsmasters_options_element_sections_filter', happy_events_options_element_sections(), $tab);
		$cmsmasters_page_fields = apply_filters('cmsmasters_options_element_fields_filter', happy_events_options_element_fields(), $tab);
		$cmsmasters_page_tabs = apply_filters('cmsmasters_options_element_tabs_filter', happy_events_options_element_tabs());
	...
	case 'cmsmasters-settings' . '-single':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'happy-events' . '_single';
		$cmsmasters_settings_page_title = esc_html__('Theme Single Posts Settings', 'happy-events');
		$cmsmasters_page_sections = apply_filters('cmsmasters_options_single_sections_filter', happy_events_options_single_sections(), $tab);
		$cmsmasters_page_fields = apply_filters('cmsmasters_options_single_fields_filter', happy_events_options_single_fields(), $tab);
		$cmsmasters_page_tabs = apply_filters('cmsmasters_options_single_tabs_filter', happy_events_options_single_tabs());
	...
	case 'cmsmasters-settings' . '-demo':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'happy-events' . '_demo';
		$cmsmasters_settings_page_title = esc_html__('Theme Settings Import/Export', 'happy-events');
		$cmsmasters_page_sections = happy_events_options_demo_sections();
		$cmsmasters_page_fields = happy_events_options_demo_fields();
		$cmsmasters_page_tabs = happy_events_options_demo_tabs();
...
function happy_events_create_settings_field($args = array()) {
	$defaults = array( 
		'id' => 		'default_field', 
		'title' => 		esc_html__('Default Field', 'happy-events'), 
		'desc' => 		esc_html__('This is a default description.', 'happy-events'), 
		'std' => 		'', 
		'type' => 		'text', 
...
function happy_events_register_settings() {
	$settings_output = happy_events_get_settings();
	...
	register_setting($cmsmasters_option_name, $cmsmasters_option_name, 'happy_events_validate_options');
	
	
	if (!empty($settings_output['cmsmasters_page_sections'])) {
		foreach ($settings_output['cmsmasters_page_sections'] as $id => $title) {
			add_settings_section($id, $title, 'happy_events_section_fn', __FILE__);
		}
	}
	...
	if (!empty($settings_output['cmsmasters_page_fields'])) {
		foreach ($settings_output['cmsmasters_page_fields'] as $option) {
			happy_events_create_settings_field($option);
		}
	}
...
function happy_events_settings_scripts() {
	wp_enqueue_style('wp-jquery-ui-dialog');
	
	
	wp_enqueue_style('happy-events-settings-css', get_template_directory_uri() . '/framework/admin/settings/css/cmsmasters-theme-settings.css', array(), '1.0.0', 'screen');
	
	
	if (is_rtl()) {
		wp_enqueue_style('happy-events-settings-css-rtl', get_template_directory_uri() . '/framework/admin/settings/css/cmsmasters-theme-settings-rtl.css', array(), '1.0.0', 'screen');
	}
	...
	wp_enqueue_script('happy-events-settings-js', get_template_directory_uri() . '/framework/admin/settings/js/cmsmasters-theme-settings.js', array('jquery', 'farbtastic'), '1.0.0', true);
	...
	wp_localize_script('happy-events-settings-js', 'cmsmasters_setting', array( 
		...
		'remove_sidebar' => 	esc_html__('Do you realy want to remove this sidebar?', 'happy-events'), 
		'find' => 				esc_attr__('Find icons', 'happy-events'), 
		'remove_icon' => 		esc_html__('Do you realy want to remove this social icon?', 'happy-events'), 
		'theme_uri' => 			get_template_directory_uri(), 
		'shortname' => 			'happy-events', 
		'done' => 				esc_html__('All theme settings were imported successfully.', 'happy-events'), 
		'fail' => 				esc_html__("Error on page!!!\nPlease reload page and try again.", 'happy-events'), 
	...
	wp_enqueue_script('happy-events-settings-js-toggle', get_template_directory_uri() . '/framework/admin/settings/js/cmsmasters-theme-settings-toggle.js', array('jquery'), '1.0.0', true);
	
	wp_localize_script('happy-events-settings-js-toggle', 'cmsmasters_settings', array( 
		'shortname' => 	'happy-events' 
	));
...
function happy_events_add_menu() {
	$settings_output = happy_events_get_settings();
	...
	$add_menu_p( 
		esc_attr__('Theme Settings', 'happy-events'), 
		esc_attr__('Theme Settings', 'happy-events'), 
		'manage_options', 
		'cmsmasters-settings', 
		'happy_events_settings_page_fn', 
		'' 
	);
	...
	$cmsmasters_settings_general = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme General Settings', 'happy-events'), 
		esc_attr__('General', 'happy-events'), 
		'manage_options', 
		'cmsmasters-settings', 
		'happy_events_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_general, 'happy_events_settings_scripts');
	
	
	$cmsmasters_settings_font = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Fonts Settings', 'happy-events'), 
		esc_attr__('Fonts', 'happy-events'), 
		'manage_options', 
		'cmsmasters-settings' . '-font', 
		'happy_events_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_font, 'happy_events_settings_scripts');
	
	
	$cmsmasters_settings_color = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Color Schemes', 'happy-events'), 
		esc_attr__('Colors', 'happy-events'), 
		'manage_options', 
		'cmsmasters-settings' . '-color', 
		'happy_events_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_color, 'happy_events_settings_scripts');
	
	
	$cmsmasters_settings_element = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Elements Settings', 'happy-events'), 
		esc_attr__('Elements', 'happy-events'), 
		'manage_options', 
		'cmsmasters-settings' . '-element', 
		'happy_events_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_element, 'happy_events_settings_scripts');
	
	
	$cmsmasters_settings_single = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Single Posts Settings', 'happy-events'), 
		esc_attr__('Single Posts', 'happy-events'), 
		'manage_options', 
		'cmsmasters-settings' . '-single', 
		'happy_events_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_single, 'happy_events_settings_scripts');
	
	
	$cmsmasters_settings_demo = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Settings Import/Export', 'happy-events'), 
		esc_attr__('Import/Export', 'happy-events'), 
		'manage_options', 
		'cmsmasters-settings' . '-demo', 
		'happy_events_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_demo, 'happy_events_settings_scripts');
}


function happy_events_section_fn($desc) {
	$tab = happy_events_get_the_tab();
...
function happy_events_settings_page_fn() {
	$settings_output = happy_events_get_settings();
	$current_tab = happy_events_get_the_tab();
	
	echo '<div class="wrap">';
	
	happy_events_settings_page_header();
	...
	if ($current_tab != 'wpml') {
		...
		'<p class="submit-top">' . 
			(($current_tab != 'import' && $current_tab != 'export') ? '<input name="submit-top" type="submit" class="button button-primary button-large" value="' . esc_attr__('Save Changes', 'happy-events') . '" />' : '') . 
		'</p>';
		...
		echo '<p class="submit">' . 
				(($current_tab != 'import' && $current_tab != 'export') ? '<input name="submit" type="submit" class="button button-primary button-large" value="' . esc_attr__('Save Changes', 'happy-events') . '" />' : (($current_tab == 'import') ? '<input name="import" type="button" class="button button-primary button-large cmsmasters-demo-import" value="' . esc_attr__('Import Settings', 'happy-events') . '" />' : '')) . 
			'</p>' . 
		'</form>';
	} else {
		do_settings_sections(__FILE__);
...
function happy_events_form_field_fn($args = array()) {
	extract($args);
	...
	case 'upload':
		...
		echo '<div class="cmsmasters_upload_parent cmsmasters_select_parent">' . 
			'<input type="button" id="cmsmasters_upload_' . $id . '_button" class="cmsmasters_upload_button button button-large" value="' . esc_attr__('Choose Image', 'happy-events') . '" data-title="' . esc_attr__('Choose Image', 'happy-events') . '" data-button="' . esc_attr__('Insert Image', 'happy-events') . '" data-id="cmsmasters-media-select-frame-' . $id . '" data-classes="media-frame cmsmasters-media-select-frame' . ((!isset($description)) ? ' cmsmasters-frame-no-description' : '') . ((!isset($caption)) ? ' cmsmasters-frame-no-caption' : '') . ((!isset($align)) ? ' cmsmasters-frame-no-align' : '') . ((!isset($link)) ? ' cmsmasters-frame-no-link' : '') . ((!isset($size)) ? ' cmsmasters-frame-no-size' : '') . '" data-library="image" data-type="' . $frame . '"' . (($frame == 'post') ? ' data-state="insert"' : '') . ' data-multiple="' . $multiple . '" />' . 
			'<div class="cmsmasters_upload"' . (($image != '') ? ' style="display:block;"' : '') . '>' . 
				'<img src="' . (($image != '') ? $image : '') . '" class="cmsmasters_preview_image" alt="" />' . 
				'<a href="#" class="cmsmasters_upload_cancel admin-icon-remove" title="' . esc_attr__('Remove', 'happy-events') . '"></a>' . 
			'</div>' . 
	...
	case 'typorgaphy':
		...
		if ($system_font) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				foreach (happy_events_system_fonts_list() as $key => $value) {
					echo '<option value="' . esc_attr($key) . '"' . (($options[$id . '_system_font'] == $key) ? ' selected="selected"' : '') . '>' . esc_html($value) .'</option>';
				}
				
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_system_font">' . esc_html__('System Font', 'happy-events') . '</label>' . 
			'</div>';
		}
		...
		if ($google_font) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_google_font">' . esc_html__('Google Font', 'happy-events') . '</label>' . 
			'</div>';
		}
		...
		if ($font_size) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				' &nbsp; ' . 
				'<label for="' . $id . '_font_size_number">' . esc_html__('Font Size', 'happy-events') . ' <em>(' . esc_html__('in pixels', 'happy-events') . ')</em></label>' . 
			'</div>';
		}
		...
		if ($line_height) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				' &nbsp; ' . 
				'<label for="' . $id . '_line_height_number">' . esc_html__('Line Height', 'happy-events') . ' <em>(' . esc_html__('in pixels', 'happy-events') . ')</em></label>' . 
			'</div>';
		}
		...
		if ($font_weight) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_font_weight">' . esc_html__('Font Weight', 'happy-events') . '</label>' . 
			'</div>';
		}
		...
		if ($font_style) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_font_style">' . esc_html__('Font Style', 'happy-events') . '</label>' . 
			'</div>';
		}
		...
		if ($text_transform) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				foreach (happy_events_text_transform_list() as $key => $value) {
					echo '<option value="' . esc_attr($key) . '"' . (($options[$id . '_text_transform'] == $key) ? ' selected="selected"' : '') . '>' . esc_html($value) .'</option>';
				}
				
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_text_transform">' . esc_html__('Text Transform', 'happy-events') . '</label>' . 
			'</div>';
		}
		...
		if ($text_decoration) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				foreach (happy_events_text_decoration_list() as $key => $value) {
					echo '<option value="' . esc_attr($key) . '"' . (($options[$id . '_text_decoration'] == $key) ? ' selected="selected"' : '') . '>' . esc_html($value) .'</option>';
				}
				
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_text_decoration">' . esc_html__('Text Decoration', 'happy-events') . '</label>' . 
			'</div>';
		}
	...
	case 'sidebar':
		...
		'<div class="sidebar_management">' . 
			'<p>' . 
				...
				'<input class="button" type="button" id="add_sidebar" value="' . esc_attr__('Add Sidebar', 'happy-events') . '" />' . 
			'</p>' . 
			'<div></div>' . 
			'<ul>';
	...
	case 'social':
		...
		'<div class="icon_management">' . 
			'<p>' . 
				...
				'<input id="' . $id . '_button" class="cmsmasters_icon_choose_button button" type="button" value="' . esc_attr__('Choose icon', 'happy-events') . '" />' . 
				'<a href="#" class="cmsmasters_remove_icon admin-icon-remove" title="' . esc_attr__('Cancel changes', 'happy-events') . '"></a>' . 
			'</p>' . 
			'<span class="cl"><br /></span>' . 
			'<span class="icon_upload_link" style="display:none;">' . 
				'<label for="new_icon_color">' . 
					'<span class="cmsmasters_col_label">' . esc_html__('Icon Color', 'happy-events') . '</span>' . 
				...
				'<label for="new_icon_hover">' . 
					'<span class="cmsmasters_col_label">' . esc_html__('Icon Hover Color', 'happy-events') . '<br /><em>' . esc_html__('if not selected - Icon Color will be used', 'happy-events') . '</em></span>' . 
				...
				'<label for="new_icon_link">' . 
					...
					esc_html__('Icon Link', 'happy-events') . 
				'</label>' . 
				'<label for="new_icon_title">' . 
					'<input class="all-options" type="text" id="new_icon_title" /> ' . 
					esc_html__('Icon Title', 'happy-events') . 
				'</label>' . 
				'<label for="new_icon_target">' . 
					'<input type="checkbox" id="new_icon_target" value="true" /> ' . 
					esc_html__('Open link in a new tab/window?', 'happy-events') . 
				'</label>' . 
			'</span>' . 
			'<span class="cl"></span>' . 
			'<input class="button button-primary" type="button" id="add_icon" value="' . esc_attr__('Add Icon', 'happy-events') . '" />' . 
			'<input class="button button-primary" type="button" id="edit_icon" value="' . esc_attr__('Save Icon', 'happy-events') . '" />' . 
			'<ul>';
			...
			if (isset($options[$id]) && is_array($options[$id])) {
				foreach($options[$id] as $icon) {
					...
					echo '<li>' . 
						...
						'</div>' . 
						'<a href="#" class="icon_del admin-icon-remove" title="' . esc_attr__('Remove', 'happy-events') . '"></a> ' . 
...
function happy_events_validate_options($input) {
	$valid_input = array();
	...
	foreach ($options as $option) {
		...
		case 'text':
			...
			case 'numeric':
				...
				$valid_input[$option['id']] = (is_numeric($input[$option['id']])) ? $input[$option['id']] : esc_html__('Expecting a Numeric value!', 'happy-events');
				
				if (is_numeric($input[$option['id']]) == false) {
					add_settings_error(
						$option['id'],
						'happy-events' . '_txt_numeric_error',
						esc_html__('Expecting a Numeric value! Please fix.', 'happy-events'),
						'error'
					);
				}
			...
			case 'multinumeric':
				...
				if ($input[$option['id']] != '') {
					$valid_input[$option['id']] = (preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) == 1) ? $input[$option['id']] : esc_html__('Expecting comma separated numeric values', 'happy-events');
				} else {
				...
				if ($input[$option['id']] != '' && preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) != 1) {
					add_settings_error(
						$option['id'],
						'happy-events' . '_txt_multinumeric_error',
						esc_html__('Expecting comma separated numeric values! Please fix.','happy-events'),
						'error'
					);
				}
			...
			case 'email':
				...
				if ($input[$option['id']] != '') {
					$valid_input[$option['id']] = (is_email($input[$option['id']]) !== false) ? $input[$option['id']] : esc_html__('Invalid email! Please re-enter!', 'happy-events');
				} elseif ($input[$option['id']] == '') {
					$valid_input[$option['id']] = esc_html__('This setting field cannot be empty! Please enter a valid email address.', 'happy-events');
				}
				
				if (is_email($input[$option['id']]) == false || $input[$option['id']] == '') {
					add_settings_error(
						$option['id'],
						'happy-events' . '_txt_email_error',
						esc_html__('Please enter a valid email address.', 'happy-events'),
						'error'
					);
				}
		...
		case 'number':
			$input[$option['id']] = trim($input[$option['id']]);
			$valid_input[$option['id']] = (is_numeric($input[$option['id']])) ? $input[$option['id']] : esc_html__('Number!', 'happy-events');
...
function happy_events_show_msg($message, $msgclass = 'info') {
	echo '<div id="message" class="' . $msgclass . '">' . $message . '</div>';
}


function happy_events_admin_msgs() {
	global $pagenow;
	...
	if ($pagenow == 'admin.php' && current_user_can('manage_options') && $cmsmasters_settings_pg !== false && !empty($set_errors)) {
		if ($set_errors[0]['code'] == 'settings_updated' && isset($_GET['settings-updated'])) {
			happy_events_show_msg('<p><strong>' . $set_errors[0]['message'] . '</strong></p>', 'updated');
		} else {
			foreach ($set_errors as $set_error) {
				happy_events_show_msg('<p class="setting-error-message" title="' . esc_attr($set_error['setting']) . '">' . $set_error['message'] . '</p>', 'error');
			}
		}
	}
}

add_action('admin_notices', 'happy_events_admin_msgs');


function happy_events_add_global_options() {
	$cmsmasters_option_names = array( 
		array( 
			'cmsmasters_options_' . 'happy-events' . '_general', 
			happy_events_options_general_fields('general') 
		), 
		array( 
			'cmsmasters_options_' . 'happy-events' . '_bg', 
			happy_events_options_general_fields('bg') 
		), 
		array( 
			'cmsmasters_options_' . 'happy-events' . '_header', 
			happy_events_options_general_fields('header') 
		), 
		array( 
			'cmsmasters_options_' . 'happy-events' . '_content', 
			happy_events_options_general_fields('content') 
		), 
		array( 
			'cmsmasters_options_' . 'happy-events' . '_footer', 
			happy_events_options_general_fields('footer') 
		), 
		array( 
			'cmsmasters_options_' . 'happy-events' . '_font_content', 
			happy_events_options_font_fields('content') 
		), 
		array( 
			'cmsmasters_options_' . 'happy-events' . '_font_link', 
			happy_events_options_font_fields('link') 
		), 
		array( 
			'cmsmasters_options_' . 'happy-events' . '_font_nav', 
			happy_events_options_font_fields('nav') 
		), 
		array( 
			'cmsmasters_options_' . 'happy-events' . '_font_heading', 
			happy_events_options_font_fields('heading') 
		), 
		array( 
			'cmsmasters_options_' . 'happy-events' . '_font_other', 
			happy_events_options_font_fields('other') 
		), 
		array( 
			'cmsmasters_options_' . 'happy-events' . '_element_sidebar', 
			happy_events_options_element_fields('sidebar') 
		), 
		array( 
			'cmsmasters_options_' . 'happy-events' . '_element_icon', 
			happy_events_options_element_fields('icon') 
		), 
		array( 
			'cmsmasters_options_' . 'happy-events' . '_element_lightbox', 
			happy_events_options_element_fields('lightbox') 
		), 
		array( 
			'cmsmasters_options_' . 'happy-events' . '_element_sitemap', 
			happy_events_options_element_fields('sitemap') 
		), 
		array( 
			'cmsmasters_options_' . 'happy-events' . '_element_error', 
			happy_events_options_element_fields('error') 
		), 
		array( 
			'cmsmasters_options_' . 'happy-events' . '_element_code', 
			happy_events_options_element_fields('code') 
		), 
		array( 
		...
		array( 
			...
			happy_events_options_single_fields('post') 
		), 
		array( 
			'cmsmasters_options_' . 'happy-events' . '_single_project', 
			happy_events_options_single_fields('project') 
		), 
		array( 
			'cmsmasters_options_' . 'happy-events' . '_single_profile', 
			happy_events_options_single_fields('profile') 
		), 
		array( 
			'cmsmasters_options_' . 'happy-events' . '_single_campaign', 
			happy_events_options_single_fields('campaign') 
		) 
	);
	...
	foreach (happy_events_all_color_schemes_list() as $key => $value) {
		array_push($cmsmasters_option_names, array( 
			'cmsmasters_options_' . 'happy-events' . '_color_' . $key, 
			happy_events_options_color_fields($key) 
		));
	}
		...
		if ($cmsmasters_option_name[1] !== '') {
			foreach ($cmsmasters_option_name[1] as $selected_option) {
				if ( 
					...
					$selected_option['id'] !== 'happy-events' . '_social_icons' 
				) {
...
function happy_events_get_global_options() {
	...
	foreach (happy_events_all_color_schemes_list() as $key => $value) {
		array_push($cmsmasters_option_names, 'cmsmasters_options_' . 'happy-events' . '_color_' . $key);
	}
