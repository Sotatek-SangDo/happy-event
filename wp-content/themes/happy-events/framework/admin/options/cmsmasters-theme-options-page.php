...
if (!function_exists('happy_events_get_custom_page_meta_fields')) {
function happy_events_get_custom_page_meta_fields() {
	$cmsmasters_option = happy_events_get_global_options();
	
	
	$cmsmasters_global_layout = (isset($cmsmasters_option['happy-events' . '_layout']) && $cmsmasters_option['happy-events' . '_layout'] !== '') ? $cmsmasters_option['happy-events' . '_layout'] : 'r_sidebar';
	...
	$cmsmasters_global_header_overlaps = (isset($cmsmasters_option['happy-events' . '_header_overlaps']) && $cmsmasters_option['happy-events' . '_header_overlaps'] !== '') ? (($cmsmasters_option['happy-events' . '_header_overlaps'] == 1) ? 'true' : 'false') : 'false';
	...
	$custom_page_meta_fields = array( 
		...
		array( 
			'label'	=> esc_html__('Page Layout', 'happy-events'), 
			'desc'	=> '', 
			'id'	=> 'cmsmasters_layout', 
			'type'	=> 'radio_img', 
			'hide'	=> '', 
			...
			'options' => array( 
				'r_sidebar' => array( 
					...
					'label' => esc_html__('Right Sidebar', 'happy-events'), 
					'value'	=> 'r_sidebar' 
				), 
				'l_sidebar' => array( 
					'img'	=> get_template_directory_uri() . '/framework/admin/inc/img/sidebar_l.jpg', 
					'label' => esc_html__('Left Sidebar', 'happy-events'), 
					'value'	=> 'l_sidebar' 
				), 
				'fullwidth' => array( 
					'img'	=> get_template_directory_uri() . '/framework/admin/inc/img/fullwidth.jpg', 
					'label' => esc_html__('Full Width', 'happy-events'), 
					'value'	=> 'fullwidth' 
				) 
			) 
		), 
		array( 
			'label'	=> esc_html__('Header Overlaps Content', 'happy-events'), 
		...
		array( 
			'label'	=> esc_html__('Disabled Heading Block', 'happy-events'), 
			'desc'	=> esc_html__('Use this checkbox to disable heading block', 'happy-events'), 
		...
		array( 
			'label'	=> esc_html__('Choose Right\Left Sidebar', 'happy-events'), 
			'desc'	=> '', 
