...
if (
	$cmsmasters_option['happy-events' . '_profile_post_cat'] || 
	$cmsmasters_option['happy-events' . '_profile_post_comment'] || 
	(
...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_open_profile'); ?>>
	...
	if ($cmsmasters_profile_title == 'true') {
		echo '<header class="cmsmasters_profile_header entry-header">';
			happy_events_profile_title_nolink(get_the_ID(), 'h1', $cmsmasters_profile_subtitle, 'h4');
		echo '</header>';
	}
	...
	echo '<div class="profile_content' . (($profile_sidebar == 'true') ? ' with_sidebar' : '') . '">';
		...
		if ($cmsmasters_option['happy-events' . '_profile_post_nav_box']) {
		...
		echo '<div class="profile_sidebar">';
			...
			if ($profile_details == 'true') {
				echo '<div class="profile_details entry-meta">' . 
					...
					happy_events_get_profile_likes('post');
					
					happy_events_get_profile_comments('post');
					
					happy_events_get_profile_features('details', $cmsmasters_profile_features, false, 'h3', true);
					
					happy_events_get_profile_category(get_the_ID(), 'pl-categs', 'post');
					
				echo '</div>';
			}
			
			
			happy_events_get_profile_features('features', $cmsmasters_profile_features_one, $cmsmasters_profile_features_one_title, 'h3', true);
			
			happy_events_get_profile_features('features', $cmsmasters_profile_features_two, $cmsmasters_profile_features_two_title, 'h3', true);
			
			happy_events_get_profile_features('features', $cmsmasters_profile_features_three, $cmsmasters_profile_features_three_title, 'h3', true);
			
			
			happy_events_profile_social_icons($cmsmasters_profile_social, esc_html__('Socials', 'happy-events'), 'h3');
