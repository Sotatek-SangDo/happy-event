...
$excerpt = (in_array('excerpt', $cmsmasters_metadata) && happy_events_slider_post_check_exc_cont('post')) ? true : false;
	...
	<?php		
		...
		echo '<div class="cmsmasters_post_top_wrap">' . 
			...
			if ($likes || $comments || $date) {
				...
				$date ? happy_events_get_slider_post_date() : '';
			
				$comments ? happy_events_get_slider_post_comments('post') : '';
			
				$likes ? happy_events_slider_post_like('post') : '';
			...
			if (!post_password_required() && has_post_thumbnail()) {
				echo '<div class="cmsmasters_date_img_wrap">';
					...
					happy_events_thumb_rollover(get_the_ID(), 'cmsmasters-service-thumb', false, false, false, false, false, false, false, false, true, false, false);
		...
		echo '<div class="cmsmasters_post_cont_wrap">';
			...
			$title ? happy_events_slider_post_heading(get_the_ID(), 'post', 'h2') : '';
			
			
			if ($author || $categories) {
				echo '<div class="cmsmasters_post_cont_info entry-meta">';
					
					$author ? happy_events_get_slider_post_author('post') : '';
					
					$categories ? happy_events_get_slider_post_category(get_the_ID(), 'category', 'post') : '';
					
				echo '</div>';
			}
			
			
			$excerpt ? happy_events_slider_post_exc_cont('post') : '';
			
			
			if ($more) {
				echo '<footer class="cmsmasters_post_footer entry-meta">';
					
					$more ? happy_events_slider_post_more(get_the_ID()) : '';
