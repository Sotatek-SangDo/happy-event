...
function happy_events_post_heading($cmsmasters_id, $tag = 'h1', $show = true) { 
	$out = '';
...
function happy_events_post_title_nolink($cmsmasters_id, $tag = 'h1', $show = true) { 
	$out = '';
...
function happy_events_get_post_date($template_type = 'page', $layout_type = 'default', $show = true) {
	if ($template_type == 'page') {
		...
		if ($cmsmasters_option['happy-events' . '_blog_post_date']) {
			$out .= '<span class="cmsmasters_post_date cmsmasters_theme_icon_date">' . 
				'<abbr class="published" title="' . esc_attr(get_the_date()) . '">' . 
					esc_html(get_the_date()) . 
				'</abbr>' . 
...
function happy_events_get_post_author($template_type = 'page', $show = true) {
	if ($template_type == 'page') {
		$out = '<span class="cmsmasters_post_author">' . 
			esc_html__('by', 'happy-events') . ' ' . 
			'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Posts by', 'happy-events') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" class="vcard author">' . 
		...
		if ($cmsmasters_option['happy-events' . '_blog_post_author']) {
			$out .= '<span class="cmsmasters_post_author">' . 
				esc_html__('by', 'happy-events') . ' ' . 
				'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Posts by', 'happy-events') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" class="vcard author">' . 
					'<span class="fn" rel="author">' . esc_html(get_the_author_meta('display_name')) . '</span>' . 
...
function happy_events_get_post_category($cmsmasters_id, $taxonomy, $template_type = 'page', $show = true) {
	$out = '';
	...
	if (get_the_terms($cmsmasters_id, $taxonomy)) {
		if ($template_type == 'page') {
			...
			if ($cmsmasters_option['happy-events' . '_blog_post_cat']) {
				$out .= '<span class="cmsmasters_post_category">' . 
					esc_html__('in', 'happy-events') . ' ' . 
					happy_events_get_the_category_list($cmsmasters_id, $taxonomy, ', ') . 
				'</span>';
...
function happy_events_get_post_tags($show = true) {
	if (get_the_tags()) {
		$cmsmasters_option = happy_events_get_global_options();
		$out = '';
		
		if ($cmsmasters_option['happy-events' . '_blog_post_tag']) {
...
function happy_events_post_exc_cont($show = true) {
...
function happy_events_get_post_likes($template_type = 'page', $show = true) {
	$out = '';
	...
	if ($template_type == 'page') {
		...
		if ($cmsmasters_option['happy-events' . '_blog_post_like']) {
			$out = cmsmasters_like('cmsmasters_post_likes');
		}
	}
...
function happy_events_get_post_comments($template_type = 'page', $show = true) {
	$out = '';
	
	
	if (comments_open()) {
		if ($template_type == 'page') {
			$out = happy_events_get_comments('cmsmasters_post_comments');
		} elseif ($template_type == 'post') {
			$cmsmasters_option = happy_events_get_global_options();
			
			if ($cmsmasters_option['happy-events' . '_blog_post_comment']) {
				$out = happy_events_get_comments('cmsmasters_post_comments');
			}
		}
	}
...
function happy_events_post_more($cmsmasters_id, $show = true) {
	$cmsmasters_post_read_more = get_post_meta($cmsmasters_id, 'cmsmasters_post_read_more', true);
	
	
	if ($cmsmasters_post_read_more == '') {
		$cmsmasters_post_read_more = esc_html__('Read More', 'happy-events');
	}
...
function happy_events_post_format_icon_placeholder($cmsmasters_id, $format = 'std', $show = true) {
	$out = '<figure>' . 
