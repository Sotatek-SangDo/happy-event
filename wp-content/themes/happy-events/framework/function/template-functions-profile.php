...
function happy_events_profile_heading($cmsmasters_id, $tag = 'h5', $sub_title = false, $tag_sub = 'h5', $show = true) { 
...
function happy_events_profile_title_nolink($cmsmasters_id, $tag = 'h5', $sub_title = false, $tag_sub = 'h5', $show = true) { 
...
function happy_events_profile_exc_cont($show = true) {
...
function happy_events_get_profile_category($cmsmasters_id, $taxonomy, $template_type = 'page', $show = true) {
	$out = '';
	...
	if (get_the_terms($cmsmasters_id, $taxonomy)) {
		if ($template_type == 'page') {
			...
			if ($cmsmasters_option['happy-events' . '_profile_post_cat']) {
				$out .= '<div class="profile_details_item">' . 
					'<div class="profile_details_item_title">' . esc_html__('Categories', 'happy-events') . ':' . '</div>' . 
...
function happy_events_get_profile_likes($template_type = 'page', $show = true) {
	$out = '';
	...
	if ($template_type == 'page') {
		...
		if ($cmsmasters_option['happy-events' . '_profile_post_like']) {
			$out = '<div class="profile_details_item">' . 
				'<div class="profile_details_item_title">' . esc_html__('Likes', 'happy-events') . ':' . '</div>' . 
...
function happy_events_get_profile_comments($template_type = 'page', $show = true) {
	$out = '';
	
	
	if (comments_open()) {
		if ($template_type == 'page') {
			$out = happy_events_get_comments('cmsmasters_profile_comments');
		} elseif ($template_type == 'post') {
			$cmsmasters_option = happy_events_get_global_options();
			
			if ($cmsmasters_option['happy-events' . '_profile_post_comment']) {
				$out .= '<div class="profile_details_item">' . 
					'<div class="profile_details_item_title">' . esc_html__('Comments', 'happy-events') . ':' . '</div>' . 
					'<div class="profile_details_item_desc details_item_desc_comments">' . 
						happy_events_get_comments('cmsmasters_profile_comments') . 
					'</div>' . 
				'</div>';
			}
		}
...
function happy_events_get_profile_features($features_position = 'features', $features = '', $features_title = false, $tag = 'h2', $show = true) {
	if (
		(
			!empty($features[0][0]) && 
			!empty($features[0][1])
...
function happy_events_profile_social_icons($social_icons, $title_box = false, $tag = 'h2', $show = true) {
	if ($social_icons != '') {
