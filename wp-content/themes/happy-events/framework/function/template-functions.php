...
function happy_events_get_page_classes($cmsmasters_option, $classes = false) {
	$browser = new Browser();
	if (
	...
	echo 'cmsmasters_' . $cmsmasters_option['happy-events' . '_theme_layout'] . ' ';

	if ($cmsmasters_option['happy-events' . '_fixed_header']) {
		echo 'fixed_header ';
	}

	if ($cmsmasters_option['happy-events' . '_header_top_line']) {
		echo 'enable_header_top ';
	}

	if ($cmsmasters_option['happy-events' . '_header_styles'] != 'default') {
		echo 'enable_header_bottom ';
	}

	if ($cmsmasters_option['happy-events' . '_header_styles'] == 'r_nav') {
		echo 'enable_header_right ';
	}

	if ($cmsmasters_option['happy-events' . '_header_styles'] == 'c_nav') {
	...
	if ($cmsmasters_header_overlaps == '') {
		$cmsmasters_header_overlaps = $cmsmasters_option['happy-events' . '_header_overlaps'];
	}
...
function happy_events_header_top($cmsmasters_option) {
	if ($cmsmasters_option['happy-events' . '_header_top_line']) {
		echo '<div class="header_top" data-height="' . esc_attr($cmsmasters_option['happy-events' . '_header_top_height']) . '">' . 
			'<div class="header_top_outer">' . 
				'<div class="header_top_inner">';
					
					if (
						$cmsmasters_option['happy-events' . '_header_top_line_add_cont'] !== 'none' || 
						(
							CMSMASTERS_DONATIONS && 
							class_exists('Cmsmasters_Donations') && 
							isset($cmsmasters_option['happy-events' . '_header_top_line_donations_but']) && 
							$cmsmasters_option['happy-events' . '_header_top_line_donations_but'] &&
							isset($cmsmasters_option['happy-events' . '_header_top_line_donations_but_text']) && 
							$cmsmasters_option['happy-events' . '_header_top_line_donations_but_text'] != '' 
						)
					) {
						...
						if (
							$cmsmasters_option['happy-events' . '_header_top_line_add_cont'] == 'social' && 
							isset($cmsmasters_option['happy-events' . '_social_icons'])
						) {
							happy_events_social_icons();
						} elseif (
							$cmsmasters_option['happy-events' . '_header_top_line_add_cont'] == 'nav' && 
						...
						if (
							...
							echo '<a href="' . esc_url(get_permalink($cmsmasters_donations_form_page)) . '" class="header_top_donation_but"><span>' . esc_html($cmsmasters_option['happy-events' . '_header_top_line_donations_but_text']) . '</span></a>';
						}
					...
					if ($cmsmasters_option['happy-events' . '_header_top_line_short_info'] !== '') {
						echo '<div class="header_top_left">' . 
							'<div class="meta_wrap">' . 
								stripslashes($cmsmasters_option['happy-events' . '_header_top_line_short_info']) . 
							'</div>' . 
						'</div>';
					} 
...
function happy_events_header_mid($cmsmasters_option) {
	echo '<div class="header_mid" data-height="' . esc_attr($cmsmasters_option['happy-events' . '_header_mid_height']) . '">';
		
		
		if (
			CMSMASTERS_WOOCOMMERCE && 
			$cmsmasters_option['happy-events' . '_header_styles'] != 'c_nav'
		) {
			echo '<div class="cmsmasters_dynamic_cart_wrap">';
				
				happy_events_woocommerce_cart_dropdown(); 
		...
		echo '<div class="header_mid_outer">' . 
			...
			if (
				CMSMASTERS_DONATIONS && 
				class_exists('Cmsmasters_Donations') && 
				isset($cmsmasters_option['happy-events' . '_header_donations_but']) && 
				$cmsmasters_option['happy-events' . '_header_donations_but'] && 
				isset($cmsmasters_option['happy-events' . '_header_donations_but_text']) && 
				$cmsmasters_option['happy-events' . '_header_donations_but_text'] != '' && 
				$cmsmasters_option['happy-events' . '_header_styles'] != 'default' && 
				$cmsmasters_option['happy-events' . '_header_styles'] != 'c_nav'
			) {
				$cmsmasters_donations_form_page = get_option('cmsmasters_donations_form_page');
					...
					'<div class="header_donation_but_wrap_inner">' . 
						'<div class="header_donation_but">' . 
							'<a href="' . esc_url(get_permalink($cmsmasters_donations_form_page)) . '" class="cmsmasters_button">' . 
								'<span>' . esc_html($cmsmasters_option['happy-events' . '_header_donations_but_text']) . '</span>' . 
							'</a>' . 
						'</div>' . 
					'</div>' . 
				'</div>';
			...
			if (
				$cmsmasters_option['happy-events' . '_header_search'] && 
				$cmsmasters_option['happy-events' . '_header_styles'] != 'c_nav'
			) {
			...
			if (
				...
				if (
					$cmsmasters_option['happy-events' . '_header_add_cont'] == 'cust_html' && 
					$cmsmasters_option['happy-events' . '_header_add_cont_cust_html'] !== ''
				) {
					echo '<div class="slogan_wrap">' . 
						'<div class="slogan_wrap_inner">' . 
							'<div class="slogan_wrap_text">' . 
								stripslashes($cmsmasters_option['happy-events' . '_header_add_cont_cust_html']) . 
							'</div>' . 
						'</div>' . 
					'</div>';
				} elseif (
					$cmsmasters_option['happy-events' . '_header_add_cont'] == 'social' && 
					isset($cmsmasters_option['happy-events' . '_social_icons'])
				) {
					happy_events_social_icons();
				}
				...
				'<div class="mid_nav_wrap">' . 
					'<nav>';
						...
						$nav_args = array( 
							...
							'link_before' => 		'<span class="nav_item_wrap">', 
							'link_after' => 		'</span>', 
							'fallback_cb' => 		'happy_events_fallback_menu' 
						);
...
function happy_events_get_header_search_form($cmsmasters_option) {
	if (
		$cmsmasters_option['happy-events' . '_header_search'] && 
		$cmsmasters_option['happy-events' . '_header_styles'] != 'c_nav'
	) {
		echo "<div class=\"cmsmasters_header_search_form\">
			...
			<form method=\"get\" action=\"" . esc_url(home_url('/')) . "\">
				<div class=\"cmsmasters_header_search_form_field\">
					...
					<input type=\"search\" name=\"s\" placeholder=\"" . esc_attr__('Enter Keywords', 'happy-events') . "\" value=\"\" />
				</div>
			</form>
		</div>";
	}
...
function happy_events_header_bot($cmsmasters_option) {
	if ($cmsmasters_option['happy-events' . '_header_styles'] != 'default') {
		echo '<div class="header_bot" data-height="' . esc_attr($cmsmasters_option['happy-events' . '_header_bot_height']) . '">' . 
...
/* Get Logo Function */
function happy_events_logo() {
	$cmsmasters_option = happy_events_get_global_options();
	
	
	if ($cmsmasters_option['happy-events' . '_logo_type'] == 'text') {
		if ($cmsmasters_option['happy-events' . '_logo_title'] != '') {
			$blog_title = $cmsmasters_option['happy-events' . '_logo_title'];
		} else {
			$blog_title = (get_bloginfo('name')) ? get_bloginfo('name') : 'Happy Events';
		}
		
		
		if ($cmsmasters_option['happy-events' . '_logo_subtitle'] != '') {
			$blog_descr = $cmsmasters_option['happy-events' . '_logo_subtitle'];
		} else {
			$blog_descr = (get_bloginfo('description')) ? get_bloginfo('description') : esc_html__('Default Logo Subtitle', 'happy-events');
		}
		...
		echo '<a href="' . esc_url(home_url('/')) . '" title="' . esc_attr($blog_title) . '" class="logo">' . "\n\t" . 
			'<span class="logo_text_wrap">' . 
				...
				($cmsmasters_option['happy-events' . '_logo_subtitle'] ? '<span class="title_text">' . esc_html($blog_descr) . '</span>' : '') . 
			'</span>' . 
		'</a>';
	} else {
		...
		if ($cmsmasters_option['happy-events' . '_logo_url'] == '') {
			...
			$logo_img = explode('|', $cmsmasters_option['happy-events' . '_logo_url']);
			...
			if ($cmsmasters_option['happy-events' . '_logo_url_retina'] != '') {
				$logo_img_retina = explode('|', $cmsmasters_option['happy-events' . '_logo_url_retina']);
...
/* Logo Styles */
function happy_events_theme_logo_styles() {
	$cmsmasters_option = happy_events_get_global_options();
	...
	if ($cmsmasters_option['happy-events' . '_logo_type'] == 'text') {
		if ($cmsmasters_option['happy-events' . '_logo_custom_color']) {
			$out .= "
		...
		$header_mid_height = (($cmsmasters_option['happy-events' . '_header_mid_height'] !== '') ? $cmsmasters_option['happy-events' . '_header_mid_height'] : '108');
...
add_action('wp_enqueue_scripts', 'happy_events_theme_logo_styles');



/* Get Footer Logo Function */
function happy_events_footer_logo($cmsmasters_option) {
	if (
		$cmsmasters_option['happy-events' . '_footer_type'] == 'default' && 
		$cmsmasters_option['happy-events' . '_footer_logo']
	) {
		...
		if ($cmsmasters_option['happy-events' . '_footer_logo_url'] == '') {
			...
			$footer_logo_img = explode('|', $cmsmasters_option['happy-events' . '_footer_logo_url']);
			...
			if ($cmsmasters_option['happy-events' . '_footer_logo_url_retina'] != '') {
				$footer_logo_img_retina = explode('|', $cmsmasters_option['happy-events' . '_footer_logo_url_retina']);
...
function happy_events_get_footer_custom_html($cmsmasters_option) {
	if (
		(
			$cmsmasters_option['happy-events' . '_footer_type'] == 'default' && 
			$cmsmasters_option['happy-events' . '_footer_html'] !== ''
		) || (
			$cmsmasters_option['happy-events' . '_footer_type'] == 'small' && 
			$cmsmasters_option['happy-events' . '_footer_additional_content'] == 'text' && 
			$cmsmasters_option['happy-events' . '_footer_html'] !== ''
		)
	) {
		echo '<div class="footer_custom_html_wrap">' . 
			'<div class="footer_custom_html">' . 
				do_shortcode(stripslashes($cmsmasters_option['happy-events' . '_footer_html'])) . 
			'</div>' . 
		'</div>';
	}
}
...
function happy_events_get_footer_nav($cmsmasters_option) {
	if (
		has_nav_menu('footer') && 
		(
			(
				$cmsmasters_option['happy-events' . '_footer_type'] == 'default' && 
				$cmsmasters_option['happy-events' . '_footer_nav']
			) || (
				$cmsmasters_option['happy-events' . '_footer_type'] == 'small' && 
				$cmsmasters_option['happy-events' . '_footer_additional_content'] == 'nav'
			)
		)
	) {
...
function happy_events_get_footer_social_icons($cmsmasters_option) {
	if (
		isset($cmsmasters_option['happy-events' . '_social_icons']) && 
		(
			(
				$cmsmasters_option['happy-events' . '_footer_type'] == 'default' && 
				$cmsmasters_option['happy-events' . '_footer_social']
			) || (
				$cmsmasters_option['happy-events' . '_footer_type'] == 'small' && 
				$cmsmasters_option['happy-events' . '_footer_additional_content'] == 'social'
			)
		)
	) {
		happy_events_social_icons();
...
function happy_events_page_heading() {
	$cmsmasters_option = happy_events_get_global_options();
	...
	if (
		...
		$cmsmasters_header_overlaps = $cmsmasters_option['happy-events' . '_header_overlaps'] ? 'true' : 'false';
		
		$cmsmasters_heading_alignment = $cmsmasters_option['happy-events' . '_heading_alignment'];
		$cmsmasters_heading_scheme = $cmsmasters_option['happy-events' . '_heading_scheme'];
		
		$cmsmasters_breadcrumbs = $cmsmasters_option['happy-events' . '_breadcrumbs'] ? 'true' : 'false';
	}
							...
							if ($cmsmasters_heading == 'custom') {
								...
								if (is_search()) {
									...
									if (!empty($wp_query->found_posts)) {
										echo sprintf(esc_html(_n('1 search result for: %2$s', '%1$d search results for: %2$s', $wp_query->found_posts, 'happy-events')), $wp_query->found_posts, get_search_query());
									} else {
										echo sprintf(esc_html__('0 search results for: %s', 'happy-events'), get_search_query());
									}
								} elseif (is_archive()) {
									if (is_author()) {
										if (get_the_author_meta('first_name') != '' || get_the_author_meta('last_name') != '') {
											echo sprintf(esc_html__('Author: %1$s (%2$s %3$s)', 'happy-events'), '<span class="vcard">' . get_the_author() . '</span>', get_the_author_meta('first_name'), get_the_author_meta('last_name'));
										} else {
											echo sprintf(esc_html__('Author: %s', 'happy-events'), '<span class="vcard">' . get_the_author() . '</span>');
										}
									} else {
...
/* Get Page Heading Function */
function happy_events_theme_page_heading_styles() {
	$cmsmasters_option = happy_events_get_global_options();
	...
	if (
		...
		$cmsmasters_heading_height = $cmsmasters_option['happy-events' . '_heading_height'];
		$cmsmasters_heading_bg_color = $cmsmasters_option['happy-events' . '_heading_bg_color'];
		$cmsmasters_heading_bg_img_enable = $cmsmasters_option['happy-events' . '_heading_bg_image_enable'] ? 'true' : 'false';
		$cmsmasters_heading_bg_img = $cmsmasters_option['happy-events' . '_heading_bg_image'];
		$cmsmasters_heading_bg_rep = $cmsmasters_option['happy-events' . '_heading_bg_repeat'];
		$cmsmasters_heading_bg_att = $cmsmasters_option['happy-events' . '_heading_bg_attachment'];
		$cmsmasters_heading_bg_size = $cmsmasters_option['happy-events' . '_heading_bg_size'];
	}
...
add_action('wp_enqueue_scripts', 'happy_events_theme_page_heading_styles');



/* Get Social Icons Function */
function happy_events_social_icons() {
	$cmsmasters_option = happy_events_get_global_options();
...
<div class=\"social_wrap\">
	...
	foreach ($cmsmasters_option['happy-events' . '_social_icons'] as $cmsmasters_social_icons) {
...
function happy_events_theme_social_icons_styles() {
	$cmsmasters_option = happy_events_get_global_options();
...
add_action('wp_enqueue_scripts', 'happy_events_theme_social_icons_styles');



/* Get Posts Thumbnail Function */
function happy_events_thumb($cmsmasters_id, $type = 'post-thumbnail', $link = true, $group = false, $preload = true, $highImg = false, $fullwidth = true, $show = true, $attachment = false, $unique = false, $link_icon = false, $placeholder_icon = 'cmsmasters_theme_icon_image') {
	$args = array( 
...
function happy_events_thumb_link($cmsmasters_id, $type = 'post-thumbnail', $link = true, $group = false, $preload = true, $highImg = false, $fullwidth = true, $show = true, $attachment = false, $unique = false, $link_icon = false, $placeholder_icon = 'cmsmasters_theme_icon_image') {
	$args = array( 
		'class' => (($fullwidth) ? 'full-width' : ''), 
		'alt' => cmsmasters_title($cmsmasters_id, false), 
		'title' => cmsmasters_title($cmsmasters_id, false) 
...
function happy_events_thumb_small($cmsmasters_id, $type = 'post', $w = 100, $h = 100, $show = true) {
...
/* Get Heading Function */
function happy_events_get_comments($class = false, $show = false) { 
	$out = '';
	
	
	if (comments_open()) {
		$out .= '<span class="cmsmasters_comments' . ($class ? ' ' . $class : '') . '">' . 
			'<a class="cmsmasters_theme_icon_comment" href="' . esc_url(get_comments_link()) . '" title="' . esc_attr__('Comment on', 'happy-events') . ' ' . esc_attr(get_the_title()) . '">' . 
				'<span>' . esc_html(get_comments_number()) . ' ' . 'Comments' . '</span>' . 
			'</a>' . 
		'</span>';
	}
...
/****************************** Blog, Services, Profiles Single Functions ******************************/

/* Get Previous & Next Post Links Function */
function happy_events_prev_next_posts() {
	...
	if ($published_posts > 1) {
		...
		previous_post_link('<span class="cmsmasters_prev_post"><span>' . esc_html__('Previous', 'happy-events') . '</span>%link</span>');
		
		next_post_link('<span class="cmsmasters_next_post"><span>' . esc_html__('Next', 'happy-events') . '</span>%link</span>');
...
function happy_events_sharing_box($title_box = false, $tag = 'h4') {
	...
	echo "<aside class=\"share_posts\">
		...
		<div class=\"share_posts_inner\">
			<a href=\"https://www.facebook.com/sharer/sharer.php?display=popup&u={$page_link}\">" . esc_html__('Facebook', 'happy-events') . "</a>
			<a href=\"https://plus.google.com/share?url={$page_link}\">" . esc_html__('Google+', 'happy-events') . "</a>
			<a href=\"https://twitter.com/intent/tweet?text=" . urlencode(html_entity_decode(sprintf(esc_attr__("Check out '%s' on %s website", 'happy-events'), $social_title, $website_name), ENT_QUOTES, 'UTF-8')) . "&url={$page_link}\">" . esc_html__('Twitter', 'happy-events') . "</a>
			<a href=\"https://pinterest.com/pin/create/button/?url={$page_link}&media={$pinterest_img}&description={$social_title}\">" . esc_html__('Pinterest', 'happy-events') . "</a>
		</div>
	</aside>
";
}
...
function happy_events_author_box($title_box = false, $tag = 'h3', $author_tag = 'h4') {
	...
	if ($user_description) {
		...
		if ($user_url) {
			echo '<a href="' . esc_url($user_url) . '" title="' . esc_attr(get_the_author()) . ' ' . esc_attr__('website', 'happy-events') . '" target="_blank">' . esc_html($user_url) . '</a>';
		}
...
function happy_events_related($tag = 'h4', $box_type = false, $tgsarray = null, $items_number = 5, $pause_time = 5, $type = 'post', $taxonomy = null) {
	if ( 
		...
		echo "<aside class=\"cmsmasters_single_slider" . (($type == 'campaign') ? ' cmsmasters_single_slider_campaign' : '') . "\">
			...
			if ($type == 'post') {
				esc_html_e('More posts', 'happy-events');
			} elseif ($type == 'campaign') {
				esc_html_e('More campaigns', 'happy-events');
			} else {
				esc_html_e('More services', 'happy-events');
			}
			...
			if ($r_query->have_posts()) {
					...
					'>';
						while ($r_query->have_posts()) : $r_query->the_post();
							if ($type == 'project') {
								...
								echo "<div class=\"cmsmasters_owl_slider_item cmsmasters_single_slider_item cmsmasters_single_slider_item_service\">
									<div class=\"cmsmasters_single_slider_item_outer\">";
										...
										echo '<div class="cmsmasters_single_slider_item_top_wrap' . $cmsmasters_with_image . '">' . 
											'<div class="cmsmasters_single_slider_item_top_wrap_inner">';
												...
												if ( $cmsmasters_option['happy-events' . '_portfolio_project_icon'] && $cmsmasters_project_icon != '' ) {
													echo '<span class="cmsmasters_single_slider_item_service_icon ' . esc_attr($cmsmasters_project_icon) . '"></span>';
												} 
												
												if ( $cmsmasters_option['happy-events' . '_portfolio_project_price'] && $cmsmasters_project_price != '' ) {
													$portfolio_project_price_currency = $cmsmasters_option['happy-events' . '_portfolio_project_price_currency'];
													
													$portfolio_project_price_currency_pos = (isset($cmsmasters_option['happy-events' . '_portfolio_project_price_currency_pos'])) ? $cmsmasters_option['happy-events' . '_portfolio_project_price_currency_pos'] : 'before';
													
													echo '<span class="cmsmasters_single_slider_item_service_price">' . ($portfolio_project_price_currency_pos == 'after' ? esc_html($cmsmasters_project_price) . esc_html($portfolio_project_price_currency) : esc_html($portfolio_project_price_currency) . esc_html($cmsmasters_project_price)) . '</span>';
												}
											...
											if (!post_password_required() && has_post_thumbnail()) {	
												...
												happy_events_thumb(get_the_ID(), 'cmsmasters-service-thumb', true, 'img_' . get_the_ID(), true, true, true, true, false);
												
											} else {
												
												happy_events_thumb(get_the_ID(), 'cmsmasters-service-thumb', true, false, true, false, true, true, false, false, true, 'cmsmasters_theme_icon_image');
										...
										echo "<div class=\"cmsmasters_single_slider_item_inner\">";
												...
												if (
													$cmsmasters_option['happy-events' . '_portfolio_project_participants'] && $cmsmasters_project_participants != '' || 
													$cmsmasters_option['happy-events' . '_portfolio_project_duration'] && $cmsmasters_project_duration != ''
												) {
													echo "<div class=\"cmsmasters_single_slider_item_inner_meta\">";
														if($cmsmasters_option['happy-events' . '_portfolio_project_participants'] && $cmsmasters_project_participants != '') {
															echo '<span class="cmsmasters_single_slider_item_service_participants">' . esc_html($cmsmasters_project_participants) . '</span>';
														}

														if($cmsmasters_option['happy-events' . '_portfolio_project_duration'] && $cmsmasters_project_duration != '') {
															echo '<span class="cmsmasters_single_slider_item_service_duration">' . esc_html($cmsmasters_project_duration) . '</span>';
														}
													echo "</div>";
												}
								...
								echo "<div class=\"cmsmasters_single_slider_item\">
									<div class=\"cmsmasters_single_slider_item_outer\">";
										...
										happy_events_thumb(get_the_ID(), 'cmsmasters-service-thumb', true, false, true, false, true, true, false, false, false, 'cmsmasters_theme_icon_image');
				...
				if ($type == 'post') {
					esc_html_e('No posts found', 'happy-events');
				} elseif ($type == 'campaign') {
					esc_html_e('No campaigns found', 'happy-events');
				} else {
					esc_html_e('No services found', 'happy-events');
				}
...
function happy_events_author_avatar($template_type = 'page') {
...
/* Get Category List */
function happy_events_get_the_category_list($cmsmasters_id, $taxonomy, $sep = '', $before = '', $after = '') {
...
if (!function_exists('happy_events_get_post_pings')) {

function happy_events_get_post_pings($id, $tag = 'h3') {
	$out = '';
	...
	if (pings_open($id) && sizeof($pings) > 0) {
		$out .= '<aside class="cmsmasters_pings_list">' . "\n" .
			'<' . esc_html($tag) . '>' . esc_html__('Trackbacks and Pingbacks', 'happy-events') . '</' . esc_html($tag) . '>' . "\n" .
