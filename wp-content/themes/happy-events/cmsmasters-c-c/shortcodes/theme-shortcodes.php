...
function happy_events_instagram_feed_shortcodes($shortcodes) {
...
add_filter('cmsmasters_custom_shortcodes_filter', 'happy_events_instagram_feed_shortcodes');
 ...
 */
function cmsmasters_service_search($atts, $content = null) { 
    $new_atts = apply_filters('cmsmasters_service_search_atts_filter', array( 
		'keywords_text' => 			'', 
		'category_text' => 			'', 
		'min_text' => 				'', 
		'max_text' => 				'', 
	...
	$out .= '<div class="search_bar_wrap cmsmasters_service_search' . 
		(($classes != '') ? ' ' . $classes : '') . 
		'"' . 
		...
		'<form method="get" action="' . esc_url(home_url('/')) . '" class="cmsmasters_row_margin">' . 
			'<p class="search_field cmsmasters_column one_fourth">' . 				
				...
				'<input name="s" placeholder="' . esc_attr__('Enter keywords', 'happy-events') . '" value="" type="search" />' . 
			'</p>' . 
			'<p class="search_field cmsmasters_column one_fourth">' . 
				'<label for="category">' . ($category_text != '' ? $category_text : '') . '</label>' . 
				'<select name="category">' . 
					'<option value="">' . esc_html__('All categories', 'happy-events') . '</option>';
				...
				$out .= '</select>' . 
			'</p>' . 
			'<p class="search_field cmsmasters_service_search_min cmsmasters_column one_fourth">' .
				'<label class="cmsmasters_service_search_min_price_max" for="price_max">' . ($max_text != '' ? $max_text : '') . '</label>' .
...
function happy_events_service_search_shortcodes($shortcodes) {
	$shortcodes[] = 'cmsmasters_service_search';
	
	
	return $shortcodes;
}

add_filter('cmsmasters_custom_shortcodes_filter', 'happy_events_service_search_shortcodes');
