...
if (post_password_required()) { 
	echo '<p class="nocomments">' . esc_html__('This post is password protected. Enter the password to view reviews.', 'happy-events') . '</p>';
...
if (comments_open()) {
	if (have_comments()) {
		echo '<aside id="comments" class="post_comments">' . "\n" . 
			...
			comments_number(esc_attr__('No Reviews', 'happy-events'), esc_attr__('Review', 'happy-events'), esc_attr__('Reviews', 'happy-events'));		
		...
		if (get_previous_comments_link() || get_next_comments_link()) {
			...
			previous_comments_link('&larr; ' . esc_attr__('Older Reviews', 'happy-events'));
			...
			next_comments_link(esc_attr__('Newer Reviews', 'happy-events') . ' &rarr;');
	...
	comment_form(array( 
		...
		'comment_field' => 		'<p class="comment-form-comment">' . 
									'<label for="comment">' . esc_html__('Your review:', 'happy-events') . '</label>' . 
									'<textarea name="comment" id="comment" cols="67" rows="2" placeholder="' . esc_html__('Message', 'happy-events') . '"></textarea>' . 
								'</p>', 
		'must_log_in' => 		'<p class="must-log-in">' . 
									...
									'</a> ' 
									. esc_html__('to post a review', 'happy-events') . 
								'.</p>' . "\n", 
		...
		'id_form' => 				'commentform', 
		'id_submit' => 				'submit', 
		'title_reply' => 			esc_attr__('Leave a Review', 'happy-events'), 
		'title_reply_to' => 		esc_attr__('Leave your review to', 'happy-events'), 
		'cancel_reply_link' => 		esc_attr__('Cancel Review', 'happy-events'), 
		'label_submit' => 			esc_attr__('Add Review', 'happy-events') 
	));
}
